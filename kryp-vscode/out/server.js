...
connection.onCompletion(() => {
    ...
    const items = [
        ...keywords.map(k => ({ label: k, kind: node_1.CompletionItemKind.Keyword })),
        ...consoleMethods.map(m => ({ label: `console.${m}`, kind: node_1.CompletionItemKind.Method })),
        ...execLangs.map(l => ({ label: `var:${l}`, kind: node_1.CompletionItemKind.Constant }))
    ];
    return items;
});
connection.onHover(params => {
    ...
    if (docs[word])
        return { contents: { kind: node_1.MarkupKind.Markdown, value: docs[word] } };
    return null;
});
