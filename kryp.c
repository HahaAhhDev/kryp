...
static void buf_init(Buffer *b) { b->cap = 8192; b->buf = malloc(b->cap); b->len = 0; }
...
/* ── Lexer helpers ───────────────────────────────────────── */
static int skip_ws(const char *s, int i) { while (s[i]==' '||s[i]=='\t') i++; return i; }

static int read_ident(const char *s, int i, char *out, int max) {
    int start = i;
    while (isalnum((unsigned char)s[i]) || s[i] == '_') i++;
    int len = i - start; if (len >= max) len = max - 1;
...
static char *transpile(const char *source) {
    ...
    while (i < len) {
        ...
        if (source[i] == '"') {
            i++;
            int ss = i;
            while (i < len && source[i] != '"') { if (source[i]=='\\'&&i+1<len) i++; i++; }
        ...
        if (source[i] == '=' && i+1 < len && source[i+1] == '>') {
            ...
            if (j < len && source[j] != '\n' && source[j] != '\0') buf_char(&out, ' ');
            continue;
        }

        /* Operators */
        if (source[i] == '&' && i+1 < len && source[i+1] == '&') { buf_str(&out, " and "); i+=2; continue; }
        if (source[i] == '|' && i+1 < len && source[i+1] == '|') { buf_str(&out, " or "); i+=2; continue; }

        /* Keywords */
        if (isalpha((unsigned char)source[i]) || source[i] == '_') {
            char word[64]; int wi = read_ident(source, i, word, sizeof(word)); int wlen = wi - i;

            if (wlen==2 && memcmp(word,"fn",2)==0)       { buf_str(&out,"def "); i=wi; continue; }
            if (wlen==3 && memcmp(word,"var",3)==0)      { i=wi; if(i<len&&source[i]==' ')i++; continue; }
            if (wlen==2 && memcmp(word,"if",2)==0)       { buf_str(&out,"if "); i=wi; continue; }
            if (wlen==4 && memcmp(word,"else",4)==0)     { buf_str(&out,"else"); i=wi; continue; }
            if (wlen==3 && memcmp(word,"for",3)==0)      { buf_str(&out,"for "); i=wi; continue; }
            if (wlen==5 && memcmp(word,"while",5)==0)    { buf_str(&out,"while "); i=wi; continue; }
            if (wlen==2 && memcmp(word,"in",2)==0)       { buf_str(&out," in "); i=wi; continue; }
            if (wlen==4 && memcmp(word,"true",4)==0)     { buf_str(&out,"True"); i=wi; continue; }
            if (wlen==5 && memcmp(word,"false",5)==0)    { buf_str(&out,"False"); i=wi; continue; }
            if (wlen==4 && memcmp(word,"none",4)==0)     { buf_str(&out,"None"); i=wi; continue; }

            /* pyimport module [as alias] → import module [as alias] */
            if (wlen==8 && memcmp(word,"pyimport",8)==0) {
                buf_str(&out,"import "); i=wi;
                if(i<len&&source[i]==' ')i++;
                continue;
            }
            ...
            if (wlen==7 && memcmp(word,"console",7)==0) {
                ...
                if (i<len && source[i]=='.') {
                    i++;
                    char method[32]; int mi = read_ident(source,i,method,sizeof(method));
            ...
            if (wlen==4 && memcmp(word,"exec",4)==0) {
                i = wi;
                if (i<len && source[i]=='(') {
                    i++; i = skip_ws(source,i);
                    if (strncmp(source+i,"var:",4)==0) {
                        i+=4;
                        char lang[16]; int li=read_ident(source,i,lang,sizeof(lang));
                        i=skip_ws(source,li);
                        if(source[i]==',')i++;
                        i=skip_ws(source,i);
                        if(source[i]=='"') {
                            ...
                            if(strcmp(lang,"python")==0){buf_str(&out,"exec(");emit_interp_string(&out,source,cs,i);buf_str(&out,")");}
                            else if(strcmp(lang,"js")==0){buf_str(&out,"__kryp_exec_js__(");emit_interp_string(&out,source,cs,i);buf_str(&out,")");}
                            else if(strcmp(lang,"sh")==0){buf_str(&out,"__kryp_exec_sh__(");emit_interp_string(&out,source,cs,i);buf_str(&out,")");}
                            else{buf_str(&out,"exec(");emit_interp_string(&out,source,cs,i);buf_str(&out,")");}
                            if(i<len)i++;
            ...
            /* Pass through unknown identifiers */
            buf_append(&out, word, wlen); i = wi; continue;
        }
...
static char *read_file(const char *path) {
    ...
    if (sz > MAX_SRC) { fprintf(stderr, "✗ File too large\n"); fclose(f); return NULL; }
...
static void py_exec(const char *code) {
    if (PyRun_SimpleString(code) != 0) { PyErr_Print(); fprintf(stderr,"✗ Execution failed\n"); }
}
...
static int cmd_run(const char *path) {
    ...
    if (ext && strcmp(ext, ".kryc") == 0) {
        char *data = read_file(path); if(!data)return 1;
        char *script; asprintf(&script,"import base64\nexec(base64.b64decode('''%s''').decode())\n",data);
        free(data); py_exec(script); free(script);
    } else {
        char *src = read_file(path); if(!src)return 1;
        char *py = transpile(src); free(src); if(!py)return 1;
        py_exec(py); free(py);
    }
    return 0;
}
...
static int cmd_compile(const char *path) {
    char *src = read_file(path); if(!src)return 1;
    char *py = transpile(src); free(src); if(!py)return 1;
    char *out; asprintf(&out,"%s.kryc",path);
    char *script; asprintf(&script,"import base64\nwith open('%s','w') as f:\n    f.write(base64.b64encode(b'''%s''').decode())\n",out,py);
    py_exec(script); printf("✓ Compiled to %s\n",out);
    free(py);free(out);free(script);
    return 0;
}

static int cmd_install(const char *pkg) {
    char *cmd; asprintf(&cmd,"pip install %s",pkg);
    printf("Installing %s...\n",pkg);
    int rc = system(cmd); free(cmd);
...
int main(int argc, char **argv) {
    ...
    Py_Initialize(); PyRun_SimpleString(KRYP_RUNTIME);
    int rc=0;
